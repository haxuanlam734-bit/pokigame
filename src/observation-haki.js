...
const ObservationHaki = {
    ...
    isActive: false,
    overlayElement: null,
    crackImage: null,          // PNG crack overlay image element
    debug: true,
    ...
    // ============================================================
    // OVERLAY (visual only) — PNG crack overlay integration
    // ============================================================
    ...
    toggle: function() {
        ...
        if (this.isActive) {
            ...
            // Set base opacity to 0.62 when Haki is turned on
            this._overlayBaseOpacity = 0.62;
            this.overlayElement.style.transition = 'opacity 0.22s ease-out';
            this.overlayElement.style.opacity = String(this._overlayBaseOpacity);
            
            if (OBSERVATION_VISUAL_DEBUG) {
                console.log('[HAKI UI] crack overlay ON');
            }
            ...
            if (OBSERVATION_VISUAL_DEBUG) {
                console.log('[HAKI UI] crack overlay OFF');
            }
        ...
        if (this._overlayPulseTimer > 0) {
            ...
            if (this._overlayPulseTimer <= 0) {
                ...
                if (this.overlayElement && this.isActive) {
                    // Return to base opacity (0.62) after pulse with smooth transition
                    this.overlayElement.style.transition = 'opacity 0.08s ease-out';
                    this.overlayElement.style.opacity = String(this._overlayBaseOpacity);
                    
                    // Reset transition to default after pulse completes
                    setTimeout(() => {
                        if (this.overlayElement && this.isActive) {
                            this.overlayElement.style.transition = 'opacity 0.22s ease-out';
                        }
    ...
    // ============================================================
    // TRIGGER OVERLAY PULSE — subtle crack pulse when dodging
    // ============================================================
    _triggerOverlayPulse: function() {
        if (!this.overlayElement || !this.isActive) return;

        // Pulse from base opacity (0.62) to pulse opacity (0.76)
        const pulseOpacity = 0.76;  // Subtle pulse from 0.62 to 0.76

        // Use quick transition for pulse
        this.overlayElement.style.transition = 'opacity 0.05s ease-out';
        this.overlayElement.style.opacity = String(pulseOpacity);
        this._overlayPulseTimer = 100;  // 100ms pulse duration
        
        if (OBSERVATION_VISUAL_DEBUG) {
            console.log('[HAKI UI] crack pulse');
        }
    },
